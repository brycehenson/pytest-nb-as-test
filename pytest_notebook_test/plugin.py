...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    parser.addini(
        "notebook_disable_line_magics",
        default="true",
        help="Comment out lines starting with '%'.",
    )
    parser.addini(
...
class NotebookFile(pytest.File):
    ...
    def collect(self) -> Iterable[pytest.Item]:
        ...
        disable_magics_str = _resolve_option(
            config, "notebook_disable_line_magics", default="true"
        )
        disable_magics = _parse_bool(str(disable_magics_str))
        keep_generated = _resolve_option(
            config, "notebook_keep_generated", default="onfail"
        )
        ...
        for idx, cell_source, must_raise in selected:
            # add marker comment
            code_lines.append(
                indent + f"## notebook-test notebook={self.path.name} cell={idx}"
            )
            ...
            if must_raise:
                ...
                code_lines.append(
                    indent + "print(type(excinfo.value).__name__, str(excinfo.value))"
                )
            else:
                for line in transformed.splitlines():
                    code_lines.append(indent + line)
