...
## Directive language
Directives are controlled with comments in notebook code cells. A
...
Use inline directives to include or skip cells, require exceptions, or
set timeouts.
Flags must appear at most once per cell.  Boolean
...
the notebook.  The initial value of `test_all_cells` comes from
configuration (CLI or `pytest.ini`/`pyproject.toml`), defaulting to `True`.
...
absent, the cell inherits the `test_all_cells` value.  
...
IPython magics and shell escapes are always commented out (lines starting with `%` or `!`). If a cell magic (such as
`%%bash`) appears in a cell, the entire cell is commented out to avoid
executing non-Python content.
...
Use `pytest --help` to view all available options.  The list below is
the full set of command line options added by this plugin.
